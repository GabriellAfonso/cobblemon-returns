...
class CollectionService:
    ...
    def run(self) -> None:
        if not getattr(settings, 'SFTP_HOST', ''):
            ...
            self.log_repo.create(status=CollectionLog.STATUS_ERROR, message="SFTP_HOST not configured")
            return
        ...
        try:
            ...
            self.log_repo.create(status=CollectionLog.STATUS_OK, players_updated=updated)
